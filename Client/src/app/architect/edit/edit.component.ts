...
export class EditComponent {
  ...
  ngOnInit() {
    ...
    this.companyService.GetOneArchitect(this.companyId).subscribe({
      next: (res: any) => {
  ...
  constructor(private fb: FormBuilder, private companyService: ArchitectService, private router: Router, private activatedRoute: ActivatedRoute) {}
  ...
  onSubmit() {
    if (this.companyForm.valid) {
      ...
      this.companyService.UpdateArchitect(this.companyForm.value).subscribe({
        next: (msg: any) => {
          console.log(msg);
